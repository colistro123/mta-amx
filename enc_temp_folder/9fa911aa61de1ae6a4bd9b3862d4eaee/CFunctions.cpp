...
int CFunctions::amxLoadPlugin(lua_State *luaVM) {
	...
	PluginList plugin = getLoadedPluginByName(pluginName);
	if(!pluginName || plugin.module != nullptr || !isSafePath(pluginName)) {
	...
	addPluginToList(pluginName, hPlugin, nullptr);
...
int CFunctions::amxLoad(lua_State *luaVM) {
	...
	char buff[256];
	snprintf(buff, sizeof(buff), "%s/mods/deathmatch/resources/%s/%s", fs::current_path().string().c_str(), resName, amxName);
	fs::path amxPath = buff;

	//std::string stringy = amxPath.string();
